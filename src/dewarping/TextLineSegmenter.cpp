...
{
    //BinaryImage const binarized(binarizeGatos(image, 4, 3.0));
    //BinaryImage const binarized(binarizeGrad(image, 4, 0.75, 0));
    //BinaryImage const binarized(binarizeEdgeDiv(image, 4, -1.0, 1.0, 0));
    BinaryImage const binarized(binarizeEdgeDiv(grayRetinex(image, 31, 1.0), 4, 0.0, 1.0, 0));
...
std::list<std::vector<QPointF>>
                             TextLineSegmenter::refineSegmentation(
                                 GrayImage const& image, QPolygonF const& crop_area,
                                 ConnectivityMap& cmap, TaskStatus const& status, DebugImages* dbg)
{
    int const width = image.width();
    int const height = image.height();
    ...
    //BinaryImage binarized(binarizeGatos(seed, 4, 3.0));
    //BinaryImage binarized(binarizeEdgeDiv(seed, 4, 0.5, 1.0, 0));
    //BinaryImage binarized(binarizeGrad(seed, 4, 0.75, 0));
    BinaryImage binarized(binarizeEdgeDiv(grayRetinex(seed, 31, 1.0), 4, 0.0, 1.0, 0));
    seed = GrayImage();
    ...
    auto binarized_get_pixel = [binarized_data, binarized_stride](int x, int y)
    {
        ...
        return (binarized_data[binarized_stride * y + (x >> 5)] >> (31 - (x & 31))) & uint32_t(1);
    };
...
BinaryImage
TextLineSegmenter::calcPageMask(GrayImage const& no_content,
                                TaskStatus const& status, DebugImages* dbg)
{
...
TextLineSegmenter::maskTextLines(
    ...
    TaskStatus const&, DebugImages* dbg)
{
    struct MaskChecker
    {
        uint32_t const* data;
